<template>
  <div
    v-loading="isLoading"
    class="parse-box text-left"
    :class="fontClass[font]"
    element-loading-text="正在提交考试,请等待..."
  >
    <div v-show="!isSheet">
      ...
      <div class="parse-stamp flex jc-between ai-center">
        <h5>{{ examname }}</h5>
        <p>
          <span>{{ currentNum }}</span>/{{ lists.length }}
        </p>
      </div>
    ...
    <transition name="fade">
      <layer
        v-show="isLayer"
        :content="layer[layerIndex].content"
        :btn="btn[0]"
        :btn2="btn[1]"
        @close="closeLayer()"
        @btn="onSure()"
        @btn2="closeLayer"
      >
        <h3 v-if="layer[layerIndex].title" slot="title" class="countdown">
          {{ layer[layerIndex].title }}
        </h3>
      </layer>
    </transition>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      btn3: ['提交答案', '保存进度'],
      layer: [
        { title: '还剩104分钟', content: '确定结束作答并生成成绩报告吗？' },
        { title: '', content: '还有15道题未作答，确认交卷？' },
        { title: '', content: '确定结束作答并生成成绩报告吗？' }
      ],
      layerIndex: 1,
      questionType: {}, // 试题类型
      lists: [],
  ...
  methods: {
    ...
    onSelect(data) {
      ...
      for (let i = 0; i < arr.length; i++) {
        if (
          arr[i].id === data.item.id &&
          arr[i].post_type === data.item.post_type
        ) {
          this.currentNum = i + 1
          this.article = arr[i]
          this.onCloseSheet()
    ...
    onSubmit() {
      ...
      this.$nextTick(() => {
        if (this.lists.length - this.doed > 0) {
          this.layerIndex = 1
          this.layer[1].title = ''
          this.layer[1].content = `还有${
            this.lists.length - this.doed
          }道题未作答，确认交卷？`
          return
        }
    ...
    getDetail() {
      ...
      this.$api('topicExaminationDetail', {
        id: this.id
      }).then((res) => {
        if (res.code === 1) {
          this.$nextTick(() => {
            ...
            this.$set(this, 'questionType', res.data.question)
            this.$set(this, 'examname', res.data.name)
            this.article = res.data.posts[0]
            this.currentNum = 1
            this.timeParse(res.data.timing)
          })
          return
        }
        this.$message({
    ...
    difference() {
      ...
      return (
        this.Zerofilling(dayDiff * 24 + hours) +
        '-' +
        this.Zerofilling(minutes) +
        '-' +
        this.Zerofilling(seconds)
      )
    },
    // 补零
    Zerofilling(time) {
    ...
    countDownOne() {
      this.timering = window.setInterval(() => {
        ...
        this.countdown =
          this.Zerofilling(this.hours) +
          ':' +
          this.Zerofilling(this.minutes) +
          ':' +
          this.Zerofilling(this.seconds)
      }, 1000)
    }
  }
}
...
.topic-info li.active {
  background-color: #f7f9ff;
  color: #5b8dfe;
}
.topic-info li.active .icon {
